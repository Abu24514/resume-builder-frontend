      ...
      {isOpen && (
        <div className='absolute top-full w-xs p-3 mt-2 space-y-3 z-10 bg-white rounded-md border border-gray-200 shadow-sm'>
          {template.map((template) => (
            ...
            className={`relative p-3 border rounded-md cursor-pointer transition-all ${selectedTemplate === template.id ? "border-green-400 bg-green-100" : "border-gray-400 hover:bg-gray-100"}`}
            >
